    ...
    def upload_file(href):
        if href != dir_path:  # Ignorar el directorio en sí
            ...
            if response.status_code == 200:
                ...
                if today.weekday() <= 8:  # No es fin de semana
                    try:
...
# Programar el trabajo para que se ejecute de lunes a viernes entre las 07:00 am y las 18:00 pm
for i in range(8):  # De lunes a viernes
    schedule.every().day.at("06:54").do(job)
    schedule.every().day.at("07:54").do(job)
    schedule.every().day.at("08:54").do(job)
    schedule.every().day.at("09:54").do(job)
    schedule.every().day.at("10:54").do(job)
    ...
    schedule.every().day.at("16:54").do(job)
    schedule.every().day.at("17:54").do(job)
